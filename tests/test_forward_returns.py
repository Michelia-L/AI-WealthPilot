...
class TestEquity:
    def test_income_plus_growth_decimal_yield(self, monkeypatch):
        ...
        data = fetch_forward_returns(["EFA"], inflation=0.025, risk_free_rate=0.02)[
            "EFA"
        ]
    ...
    def test_missing_dividend_yield_degrades(self, monkeypatch):
        ...
        assert (
            fetch_forward_returns(["000300.SS"], inflation=0.025, risk_free_rate=0.02)[
                "000300.SS"
            ]
            is None
        )
    ...
    def test_yield_proxy_also_missing_degrades(self, monkeypatch):
        monkeypatch.setattr(forward_returns, "_fetch_info_fields", lambda ticker: None)
        assert (
            fetch_forward_returns(["000300.SS"], inflation=0.025, risk_free_rate=0.02)[
                "000300.SS"
            ]
    ...
    def test_info_fetch_failure_degrades(self, monkeypatch):
        monkeypatch.setattr(forward_returns, "_fetch_info_fields", lambda ticker: None)
        assert (
            fetch_forward_returns(["EWH"], inflation=0.025, risk_free_rate=0.02)["EWH"]
            is None
        )
...
class TestBond:
    def test_fund_yield_preferred(self, monkeypatch):
        ...
        data = fetch_forward_returns(["AGG"], inflation=0.025, risk_free_rate=0.02)[
            "AGG"
        ]
        assert data is not None
    ...
    def test_all_sources_fail_returns_none(self, monkeypatch):
        ...
        assert (
            fetch_forward_returns(["AGG"], inflation=0.025, risk_free_rate=0.02)["AGG"]
            is None
        )
...
class TestSimpleKinds:
    def test_gold_equals_inflation(self):
        data = fetch_forward_returns(["GLD"], inflation=0.025, risk_free_rate=0.02)[
            "GLD"
        ]
        assert data is not None
    ...
    def test_cash_equals_risk_free_rate(self):
        data = fetch_forward_returns(["BIL"], inflation=0.025, risk_free_rate=0.02)[
            "BIL"
        ]
        assert data is not None
    ...
    def test_unmapped_ticker_returns_none(self):
        assert (
            fetch_forward_returns(["BTC-USD"], inflation=0.025, risk_free_rate=0.02)[
                "BTC-USD"
            ]
            is None
        )
