...
**报价口径（#52 修正）**。VIX 与 MOVE 的报价语义不同。VIX 是价格波动率的百分点（20 = 20%），直接乘 0.01。MOVE 是收益率波动率的基点（73 ≈ 73bp），需先换算为价格波动率，公式为 `iv = raw × 1e-4 × duration`，久期按资产取近似常量（AGG 6.0、TLT 16.5、HYG 3.8、EMB 7.0、TIP 6.5）。早期版本对 MOVE 沿用 0.01，把 73bp 读成 73%，债券混合波动率被放大约一个数量级。HYG/EMB 的换算只覆盖利率成分，信用利差风险是已知低估。
