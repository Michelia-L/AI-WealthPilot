...
## KI-004 · 认证对外部署前的限流与撤销边界

**发现日期**：2026-09-13　**状态**：待处理；对外部署前置条件。

来源：[PR #90 账号锁定评审](https://github.com/Michelia-L/AI-WealthPilot/pull/90#discussion_r3996797736)、[会话撤销评审](https://github.com/Michelia-L/AI-WealthPilot/pull/90#discussion_r3996797739)。与 #51 的安全基础工作衔接，不能因 #72 或 #75 完成而默认这些要求已经满足。

### 当前行为与风险

- 连续五次错误密码触发五分钟账号级冷却。知道 email 的攻击者可以反复触发冷却，长期阻断该用户的新登录；已有会话不受冷却影响。
- `is_active=False` 仅暂停认证，旧会话仍存库。重新启用用户后，未过期且未退出的旧 token 可以再次使用。关闭再开启 `DEMO_MODE` 具有相同语义。
- 目前只有单会话 logout 实现永久撤销，没有面向管理员的账号禁用或全部会话撤销入口。不能将直接翻转 `is_active` 用作凭据泄露后的撤销措施。
- 哈希容量限制只控制单进程的并发内存使用。槽位满时立即返回 503 已解决共享线程等待问题，但不能替代请求限流。

### 对外部署前的验收条件

- 明确信任代理与客户端来源解析，只信任配置允许的转发头，防止伪造来源绕过限制。
- 建立账号与 IP/设备等来源相结合的限流，或有上限的渐进退避；多进程部署需共享相关状态。
- 覆盖攻击者反复提交已知 email 错误密码的场景，验证合法用户的恢复路径，避免账号被无限期锁住。
- 明确区分临时暂停和安全性禁用；后者需要原子地阻止新会话并撤销全部旧会话，例如通过事务删除或会话版本校验。
- 覆盖禁用/重启/重新启用、并发登录与撤销，以及 Demo 模式切换的测试，保证安全性禁用后旧 token 不会恢复有效。

当前范围及使用方式见 [identity-auth.md](identity-auth.md)。
