...
本章讲 `api/`——FastAPI 薄传输壳。它的存在理由一句话：**让 `src/` 的计算核心可以不知道 HTTP 的存在**。壳的职责被严格限定为三件事：参数校验（Pydantic + 路由内 fail-fast）、调用 `src/`、组装响应。红线：路由里禁止写业务逻辑。

与第 1 章的分工：第 1 章沿一次优化请求走叙事，本章把壳本身的机制与边界讲全——端点地图、持久化、SSE 任务、缓存、i18n。
...
一个隐形的正确性细节在 `api/__init__.py`：它把仓库根插到 `sys.path[0]`，所以 `main.py` 头部那个 `import api  # noqa: F401` 的「无用导入」是刻意的——**导入顺序即功能**，删掉它 `src.*` 导入就会在某些启动目录下失败。
...
9 个 router + `main.py` 的 health，共 **43 个端点**。按域分组（`*` = SSE 流式）：
...
1. 语言解析 `get_request_locale(request)`——`X-Locale` 头，未知/缺失一律回退英文
2. profile 解析（不存在 → 404）与方法约束（BL 无观点 / ERC 做空 → 422）——**全部在取行情之前**
3. 取数与求解；src 层抛出的异常按类型翻译：`KeyError` → 404、`ValueError` → 422、`TokenBudgetExceeded` → 专属 422 分支（带 spent/budget 的本地化文案）

**路由声明顺序是正确性的一部分**：`/monitoring/status` 必须声明在 `/{document_id}` 之前——`"status"` 满足 document_id 的字符集，注册晚了会被路径参数吞掉返回 404（代码注释明写）。`profiles` 的 `/questionnaire`、`/compare` 同理。
...
82 个 Pydantic 模型集中在一个文件（1116 行），按域分组：meta/行情、优化、退休、画像、顾问、IPS、监控、LLM 设置。值得说的设计：
...
| `app_settings` | `AppSettingRecord` | KV 表——FR-002 的 LLM 端点配置（`llm_base_url`/`llm_api_key`/`llm_model`） |

工程细节：`connect_args={"check_same_thread": False}`（FastAPI 在线程池里跑同步路由）；engine 在**导入时**创建（import `api.db` 即解析 DB URL）——测试经 `AIWP_DB_URL` 环境变量或 monkeypatch 重定向。

## SSE 任务机制（DB 侧）

第 1 章讲了断线重连的叙事，这里补机制细节（`api/tasks.py`，239 行）：

- **写穿透顺序**：`publish()` 给事件盖从 1 开始的 `seq`，**先写库**（read-modify-write `events_json`，截断保留最后 500 条；终态 done/error 同时翻转行状态并写 `finished_at`）**再入内存队列**——SSE drain 看到终态时它已持久化。
...
- **开机和解**：`reconcile_interrupted_tasks()` 把遗留 `running` 置 `failed`；回放时在尾部合成一条本地化「任务被中断」事件。
- **持久化失败只记日志不抛出**——DB 坏了不许弄死计算任务本身。

没有 Celery/Redis：任务在进程内跑，这套机制覆盖「断线重连、重启回放」的真实需求。**边界**：任务注册表是单进程的——多 worker 部署时任务只活在创建它的进程里。

## TTL 缓存（`api/cache.py`）

`TTLCache` 是进程内线程安全缓存，41 行：`get_or_set(key, ttl, factory)`。**factory 在锁外执行**——慢取数不阻塞其他读者，代价是并发下可能重复计算（可接受）。无容量上限、无后台清理，条目只在被访问且过期时被覆盖。注释明示：多进程部署须换共享存储（Redis）。

全包 10 个实例：行情报价 300s、分析 300s、无风险利率 3600s ×2、价格帧 300s、回测 600s ×2、收益率曲线 3600s、基金 AUM 86400s、监控 fleet 86400s。两个模式值得单独说：

- **主动失效**：曲线/AUM 缓存取到 None 时立即 `invalidate`——不让降级结果钉满整个 TTL，下一请求重试。
- **日期进 key**：fleet 状态的缓存 key 含 `date.today()`——「每日首次请求自动重算」的懒语义，没有定时器。

## i18n 消息表（`api/i18n.py`）

49 个 key、8 个分组（common/advisor/ips/monitoring/portfolio/profiles/settings/tasks），每 key 双语，zh 是 i18n 改造前的既有中文逐字保留。`msg(key, locale, **fmt)` 语义：

- 未知 key **抛 `KeyError`**——让缺失条目在测试里响亮失败
- 未知 locale **静默回退英文**——两种「未知」的处理故意相反

红线与例外：路由内禁止内联中文（新文案必须双语进表）；而 `src/` 侧的计算函数把双语文案内置在各自模块（`views.py` 的 `_VIEW_STRINGS`、`ips_workflow.py` 的 `_SAA_STRINGS` 等）——因为「src 不得 import api」，这些是文档化的有意例外。已知例外：retirement/market 路由有少数内联**英文** detail（规则约束的是中文，英文未强制入表）。

## 画像转换与遗留迁移

- **`profile_convert.py`**：API payload ↔ 存储 JSON ↔ src dataclass 的三向转换。核心规则：**问卷答案优先于手动滑杆分**——某轨答案非空时用 src 规则推导并覆盖；空答案保留手动回退。`_safe_ratio` 把 inf/nan 转成 None——JSON 无法承载 inf，「有负债无资产」的 +inf 哨兵在 API 响应里变成 None。
- **`migrate_profiles.py`**：Streamlit 时代 `data/profiles/*.json` 导入 SQLite。幂等键是 `(name, created_at)` 二元组——重跑不产生重复行；`maybe_auto_import()` 仅在画像表为空时执行。注意它 import `profiler` 用**模块属性引用**（`profiler.PROFILES_DIR`）而非 from-import——为了让测试的 monkeypatch 生效。
...
- **为什么是 SQLite 而不是 Postgres？** 单用户本地工作站：零运维、一个文件、volume 一挂就走。`user_id` 列的预留说明多用户是想过的——但那是以后的事，现在不付运维税。
- **为什么事件存纯文本 JSON 列而不是关系表？** 事件词汇表归各任务类型所有且会演进；关系表会把每次演进变成 migration。JSON 列用「读方负责解析」换掉 schema 刚性。
- **为什么 82 个模型集中一个文件而不是按域分文件？** 单一事实源 + 全局可查；文件大但分组注释清晰，且消除了跨文件循环 import 的可能。
- **为什么 fleet 每日重检用「日期进 key」而不是定时任务？** 没有调度器依赖，进程重启天然安全；代价是当日首次请求略慢（懒语义）。
...
7. `client_profiles` 为什么用 JSON 列 + 冗余索引列而不是全关系表？
8. i18n 里「未知 key 抛错、未知 locale 回退」两种相反处理各自的理由？
