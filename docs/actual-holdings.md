...
- Every snapshot is complete. Omitted assets have zero market value. Holdings must be nonnegative, finite, and have a strictly positive total; short positions and an entirely empty portfolio are not supported.
...
- Current weights equal asset market value divided by total snapshot market value. No proxy return is applied to a stored valuation, even when it is older than today. The monitor displays the source and valuation date. `as_of` remains the date the diagnostics were computed.
...
Fleet caching uses a fixed date/locale key and a snapshot-write version. Cache hits read only the indexed maximum snapshot ID; cache misses select the latest row per document in SQL. Versions replace the existing cache entry, and expired entries are evicted on subsequent cache access.
