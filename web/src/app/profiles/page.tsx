import { getProfiles, getQuestionnaire } from "@/lib/api";
...
export default async function ProfilesPage() {
  const [data, questionnaire] = await Promise.all([getProfiles(), getQuestionnaire()]);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-6 py-10">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-400">
          IPS 框架客户信息管理：基本信息、财务状况、投资目标与风险问卷
        </p>
      </header>

      <ProfilesManager initialProfiles={data?.profiles ?? null} questionnaire={questionnaire} />
    </div>
  );
}
