...
- ⚖️ **LDI 盈余优化（Sharpe-Tint）**  
  面向个人家庭的负债驱动投资：负债以固定空头进入优化，最大化单位盈余波动的盈余收益（$E(R_S) = w^\top\mu - k\,\mu_L$，$\mathrm{Var}(R_S) = w^\top\Sigma w - 2k\,w^\top c + k^2\sigma_L^2$），配置自动倾向对冲负债的债券资产。负债为现金流流，三条通道——手动负债比率/久期、客户画像目标（名义口径）、**退休收入流**（今日购买力年收入按个人通胀 preset 逐年放大，老年客户自动获得 CPI-E 风格上调）——现值**按中债国债收益率曲线逐期限折现**（tushare → akshare 级联，统一无风险利率兜底），对冲代理为久期缩放的债券 ETF（AGG / TLT / TIP / 511010 上证 5 年期国债 ETF），无需手设相关性。
- 🌐 **风险平价（等风险贡献 ERC）**  
