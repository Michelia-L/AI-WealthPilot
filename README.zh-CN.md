...
| 组合监控（`/monitoring`） | 已保存 IPS 配置及人工录入/导入的持仓快照，或保存以来的代理价格 | 实际市值或买入持有权重漂移、政策区间检查、快照对比、历史回测与归因 | 偏离诊断、建议调仓权重及可选 AI 解释 |
| 退休规划（`/retirement`） | 储蓄、投入、支出、期限及收益波动假设 | 积累/提取两阶段蒙特卡洛模拟，可选支出护栏 | 财富分位数、资金存活率与支出对比 |

持仓录入、JSON 导入、估值规则与快照存储说明见[实际持仓快照](docs/actual-holdings.md)。
...
[稳健性分析](validation/portfolio/ROBUSTNESS.md)可计算考虑权重漂移的换手率、不同交易成本假设下的净收益和权重稳定性，并比较预期收益扰动、协方差估计及训练窗口变化的影响。结果保留成本前后指标和各指标的独立排名，并记录计算假设。

[风险验证](validation/portfolio/RISK_VALIDATION.md)可比较决策时风险估计与后续实际风险，按明确的事后市场状态汇总结果，并整合历史与确定性压力情景，保留样本数量和数据来源。
...
| 回测与监控 | 回测把给定目标权重应用于历史，并非滚动重新训练策略。监控优先使用人工估值的持仓快照；未录入时依据 IPS 权重与代理收益推算漂移。快照差额可能包含现金流，不代表投资收益。不接入券商同步、不执行订单，也没有税务批次核算。 |
...
- **访问控制。** API 已提供按路由接入的身份依赖，以及受保护的 `/api/auth/me` 和 `/api/auth/logout` 端点。组织成员关系、角色和客户／画像归属已持久化，详见[归属模型](docs/client-ownership.md)。现有业务 API 仍未接入认证，也未执行组织隔离或对象级授权。提供的 Compose 配置将端口绑定到 `127.0.0.1`；部署到本机之外前，需要完成这些访问控制。身份层可通过 `python -m api.create_user` 交互式创建用户，详见[身份配置与 API 使用](docs/identity-auth.md)；Web 登录接入属于后续工作。
