...
- ⚖️ **LDI 盈余优化（Sharpe-Tint）**  
  面向个人家庭的负债驱动投资：负债以固定空头进入优化，最大化单位盈余波动的盈余收益（$E(R_S) = w^\top\mu - k\,\mu_L$，$\mathrm{Var}(R_S) = w^\top\Sigma w - 2k\,w^\top c + k^2\sigma_L^2$），配置自动倾向对冲负债的债券资产。负债双通道输入——手动负债比率/久期，或按选中客户的投资目标折现自动推导（资产基数 = 可投资资产）；负债增长率可联动个人通胀 preset（老年客户自动获得 CPI-E 风格上调）。负债风险统计量由久期缩放的债券代理（AGG / TLT / TIP）估计，无需手设相关性。
- 🌐 **风险平价（等风险贡献 ERC）**  
