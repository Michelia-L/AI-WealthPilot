...
def create_sample_market_data():
    ...
    # 生成随机收益率
    returns = np.random.multivariate_normal(daily_returns, cov_matrix, size=n_days)
...
def demo_black_litterman(returns, max_sharpe_mvo):
    ...
    for view in views:
        if view.view_type == "absolute":
            print(
                f"  Absolute: {view.asset_long} → {view.expected_return:.1%} "
                f"(confidence: {view.confidence:.0f}%)"
            )
        else:
            print(
                f"  Relative: {view.asset_long} > {view.asset_short} by "
    ...
    fig_mvo = plot_allocation_pie(
        max_sharpe_mvo["weights"], title="MVO Optimal Allocation / MVO最优配置"
    )
...
def demo_monte_carlo_simulation(bl_result):
    ...
    print(
        f"  Desired Retirement Income / 期望退休收入: ${desired_retirement_income:,.0f}/year"
    )
    ...
    print("\n" + "-" * 50)
    print(
        "ACCUMULATION PHASE (Age {} to {}) / 积累阶段（{}岁到{}岁）".format(
    ...
    print("\n" + "-" * 50)
    print(
        "DISTRIBUTION PHASE (Age {} to {}) / 分配阶段（{}岁到{}岁）".format(
...
def demo_risk_metrics(returns):
    ...
    for asset in returns.columns:
        ...
        print(
            f"  Annualized Volatility / 年化波动率: {metrics['annualized_volatility']:.2%}"
        )
        ...
        print(
            f"  95% CVaR (daily) / 95%条件在险价值（日度）: {metrics['cvar_95_daily']:.2%}"
        )
...
def main():
    ...
    # 步骤2：均值-方差优化
    fig_frontier, max_sharpe, min_vol, frontier = demo_mean_variance_optimization(
        returns
    )
    ...
    print(
        f"   - 95% VaR (daily) / 95%在险价值（日度）: {us_metrics['var_95_daily']:.2%}"
    )
